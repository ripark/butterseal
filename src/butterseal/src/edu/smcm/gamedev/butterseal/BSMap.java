...
public enum BSMap {
    ICE_CAVE_ENTRY(BSAsset.ICE_CAVE_ENTRY, "ice-cave-entry",
            new BSGameStateActor() {
                @Override
                ...
                public void update(BSGameState state) {
                    ...
                    TiledMapTile invis = BSTile.getTileForProperty(m,
                            "invisible", "true");
                    BSTile t = state.player.getFacingTile();

                    if (t.hasProperty(m, m.getLayer("instruments"),
                            "instrument", null)) {
                        state.isUsingPower = false;
                        if (!m.objectiveReached) {
                            m.objectiveReached = true;
                            state.setMusic(BSAsset.FIRST_MUSIC);
                            state.world.addRoute(BSMap.ICE_CAVE_ENTRY,
                                    BSMap.ICE_CAVE, null);
                            t.setTile(m.getLayer("instruments"), invis);
                        }
                    }
                }
        ...
        public void act(BSGameState state) {
            ...
            TiledMapTile invis = BSTile.getTileForProperty(m, "invisible",
                    "true");
                        ...
                        if (lookingAt.isContainedIn(dark)) {
                            ...
                            if (!point.getTile().equals(invis)) {
                                if (ButterSeal.DEBUG > 2) {
                                    System.out.printf("Clearing tile %d,%d%n",
        ...
        public void update(BSGameState state) {
            BSMap m = state.currentMap;
            TiledMapTileLayer light = m.getLayer("light");
            TiledMapTile beacon_on = BSTile.getTileForProperty(m, "beacon",
                    "on");
            TiledMapTile invis = BSTile.getTileForProperty(m, "invisible",
                    "true");
            ...
            if (t.hasProperty(m, m.getLayer("instruments"), "instrument", null)) {
                state.isUsingPower = false;
                if (!m.objectiveReached) {
                    m.objectiveReached = true;
                    state.setMusic(BSAsset.SECOND_MUSIC);
                    state.world.addRoute(BSMap.ICE_CAVE, BSMap.ICE_CAVE_EXIT,
                            null);
                    t.setTile(m.getLayer("instruments"), invis);
            ...
            for (int row = 0; row < m.playerLevel.getHeight(); row++) {
                for (int col = 0; col < m.playerLevel.getWidth(); col++) {
                    ...
                    if (curr.hasProperty(m, light, "light", "torch")) {
                        ...
                        for (int i = -1; i <= 1; i++) {
                            for (int j = -1; j <= 1; j++) {
                                Cell point = new BSTile(row + i, col + j)
                                        .getCell(dark);
                                if (!point.getTile().equals(invis)) {
                                    if (ButterSeal.DEBUG > 2) {
                                        System.out.printf(
                                                "Clearing tile %d,%d%n", row
                                                        + i, col + j);
                                    }
                                    point.setTile(invis);
                                }
    ...
    }), ICE_CAVE_EXIT(BSAsset.ICE_CAVE_EXIT, "ice-cave-exit",
            new BSGameStateActor() {
                @Override
                public void act(BSGameState state) {
                }
    ...
    BSTile getPlayer(String key) {
        ...
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                Map<String, String> prop = ret.getProperties(this)
                        .get("player");
                if (prop != null && prop.containsKey("player")
                        && prop.get("player").equals(key)) {
                    return ret;
                }
            }
        }
    ...
    private void SetNullsToInvisible() {
        TiledMapTile invis = BSTile.getTileForProperty(this, "invisible",
                "true");
        Cell c = new Cell();
        c.setTile(invis);
